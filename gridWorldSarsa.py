...
if __name__ == '__main__':
    ...
    import valueIterationAgents,sarsalearningAgents
    s = None
    if opts.agent == 'value':
    ...
    try:
        if not opts.manual and opts.agent == 'value':
            ...
            display.displayValues(s, message = "VALUES AFTER "+str(opts.iters)+" ITERATIONS")
            display.pause()
            display.displayQValues(s, message = "Q-VALUES AFTER "+str(opts.iters)+" ITERATIONS")
            display.pause()
    except KeyboardInterrupt:
        sys.exit(0)
    ...
    if not opts.quiet:
        if opts.manual and opts.agent == None:
            ...
            if opts.agent == 'random': displayCallback = lambda state: display.displayValues(s, state, "CURRENT VALUES")
            if opts.agent == 'value': displayCallback = lambda state: display.displayValues(s, state, "CURRENT VALUES")
            if opts.agent == 'q': displayCallback = lambda state: display.displayQValues(s, state, "CURRENT Q-VALUES")
    ...
    for episode in range(1, opts.episodes+1):
        returns += runEpisode(s, env, opts.discount, decisionCallback, displayCallback, messageCallback, pauseCallback, episode)
    if opts.episodes > 0:
        print
    ...
    if opts.agent == 'q' and not opts.manual:
        try:
            display.displayQValues(s, message = "Q-VALUES AFTER "+str(opts.episodes)+" EPISODES")
            display.pause()
            display.displayValues(s, message = "VALUES AFTER "+str(opts.episodes)+" EPISODES")
            display.pause()
        except KeyboardInterrupt:
            sys.exit(0)
